...
async def start_user_initiated_process(process_id: ObjectId, repository_id: ObjectId, actions: List[ProcessName]):
    """
    ...
    processes = await db["processes"].find({"process_id": process_id}).to_list(length=None)
